...
get_pca = function(X_mat, I_train, I_test, p_comps, NWP=NA,  U = NA, mu = NA){
    ...
    if (length(U) == 1){
        ...
        U = l_svd$u[, 1:p_comps]
        #print(U[1:20])
        ##--- Get the principal components for training dataset---
        PC_train = X_center %*% U
        colnames(PC_train) = paste0("PC", 1:dim(data.frame(U))[2])
        dt_train = data.table(date_demand[I_train, ], PC_train)

        ##----- Form the test dataset ---
        X_test = X_mat[I_test,]
        X_test_center = X_test - mu
        PC_test = X_test_center %*% U
        colnames(PC_test) = paste0("PC", 1:dim(data.frame(U))[2])
        dt_test = data.table(date_demand[I_test, ], PC_test)
    ...
    ## ---- Form principal component of NWP data ---
    if (is.null(dim(NWP)) == FALSE){
        members = dim(NWP)[4]
        NWP_PC_mat = array(NA, dim = c(500, p_comps, members))

        #Iterate through ensemble members, creating PCs
