...
###################################
#Skill by month
plot(1 - (Factor10_inter3$Results[,  mean((volume - pred_3)^2), by = month(date)][,V1]/
              Climatology$Results[, mean((volume - clima_pred)^2), by = month(date)][,V1]),
     type = 'l', ylim = c(-0.1, 0.8), xlab = 'Month', ylab = 'Skill Score',
     main = 'Skill of GAM-FL2 by Month')
axis(month.abb, side = 4, labels = month.abb, at = 1:12)
legend('topright', lty = 1, cex = 1, pch = 16, col = c('black','red', 'orange'), title = 'Skill relative to:', legend = c("Climatology", "Time Interaction", "GAM-FL1"))
abline(h = 0)
lines(1 - (Factor10_inter3$Results[,  mean((volume - pred_3)^2), by = month(date)][,V1]/
               Inter4$Result[, mean((volume - pred_1)^2), by = month(date)][,V1]), col = 'red')
lines(1 - (Factor10_inter3$Results[,  mean((volume - pred_3)^2), by = month(date)][,V1]/
               Factor10_inter3$Result[, mean((volume - pred_2)^2), by = month(date)][,V1]), col = 'orange')

#By hour
plot(1:24,1 - (Factor10_inter3$Results[,  mean((volume - pred_3)^2), by = hour][,V1]/
              Climatology$Results[, mean((volume - clima_pred)^2), by = hour][,V1]),
     type = 'l', ylim = c(-0.1, 1), xlab = 'Hour', ylab = 'Skill Score',
     main = 'Skill of GAM-FL1 and GAM-FL2 by hour')
abline(h = 0)
lines(1:24,1 - (Factor10_inter3$Results[,  mean((volume - pred_2)^2), by = hour][,V1]/
               Climatology$Results[, mean((volume - clima_pred)^2), by = hour][,V1]),col = 'orange')
legend('topright', lty = 1, cex = 1, pch = 16, col = c('black','orange'), title = 'Skill relative to Climatolog:', legend = c("GAM-FL1", "GAM-FL2"))


#Skill by init month
plot(unique(Factor10_inter3$Results$init_date),1 - (Factor10_inter3$Results[,  mean((volume - pred_3)^2), by = init_date][,V1]/
              Climatology$Results[, mean((volume - clima_pred)^2), by = init_date][,V1]),
     type = 'l', ylim = c(-1.7, 1), xlab = 'Initialization Month', ylab = 'Skill Score',
     main = 'Skill of GAM-FL1 and GAM-FL2 by Init month')
abline(h = 0)

lines(unique(Factor10_inter3$Results$init_date),1 - (Factor10_inter3$Results[,  sqrt(mean((volume - pred_2)^2)), by = init_date][,V1]/
               Climatology$Results[, sqrt(mean((volume - clima_pred)^2)), by = init_date][,V1])^2,col = 'red')

lines(unique(Factor10_inter3$Results$init_date),1 - (Factor10_inter3$Results[,  sqrt(mean((volume - pred_1)^2)), by = init_date][,V1]/
               Climatology$Results[, sqrt(mean((volume - clima_pred)^2)), by = init_date][,V1])^2,col = 'orange')
legend('topright', lty = 1, cex = 1, pch = 16, col = c('black','red', 'orange'), title = 'Skill relative to Climatology:', legend = c("GAM-FL1", "GAM-FL2", "Interaction"))
...
lin_mod = lm(V1 ~ lead_time, data = Factor10_inter3$Results[,  sqrt(mean((volume - pred_3)^2)), by = lead_time])
...
plot(date_demand[year(date)==yr & hour %in%12,.(date, climatology)], type = 'l', 
     ylim = c(-9, 17), ylab = 'Temperature (°C)', xlab = 'Date', main = 'Daily Mean Grid Temperature at 12:00 (lat 4-25, long 55-75)')
...
plot(date_demand[year(date)==yr & month%in% c(2,3),volume], col = 'black', ylim = c(30000, 65000), pch = 16, cex= 0.6)
lines(Factor10_inter3$Results[year(date)==yr & month %in% c(2,3),pred_3], col = 'red')
lines(Factor10_inter3$Results[year(date)==yr & month %in% c(2,3),pred_1], col = 'blue')
