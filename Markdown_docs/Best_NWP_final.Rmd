...
for (i in c(seq(1, 9, 1))){
    # plot(ERA_NWP_raw_loss[, mean(pinball_loss(i/10, get(paste0("NWP_PC1_q",i,"0")), PC1)), by = lead_time], col = 'red', type = 'l', main = paste0('Pinball Loss of NWP1 and Climatology (', i*10,"th percentile)"), ylab = 'Pinball Loss', xlab = 'Lead Time (6 hour-spaced )', ylim = c(0, 25))
    # lines(Clima[, mean(pinball_loss(i/10, get(paste0("clima_pred",i*10)), PC1)), by = lead_time], col = 'blue')
    # par(new = TRUE)
    # plot(1 - (ERA_NWP_raw_loss[, mean(pinball_loss(i/10, get(paste0("NWP_PC1_q",i,"0")), PC1)), by = lead_time][,V1]/Clima[, mean(pinball_loss(i/10, get(paste0("clima_pred",i*10)), PC1)), by = lead_time][,V1]), type = 'l', axes = FALSE, xlab = "", ylab = "", ylim = c(-0.1, 0.8))
    ...
    #     l = 'right'
    # } else{ l = 'topright'}
    # legend(l, col = c('red', 'blue', 'black'), legend =  c(paste0("NWP1 q",i,"0"), paste0("Climatology q",i,"0"), "Skill Score NWP1"),pch = 16,lty = c(1))
...
ERA_CLIMA = merge(Clima, ERA_NWP_raw_loss, by = c('date', 'hour', 'lead_time', 'PC1'))
...
```{r}
plot(Interval_Mod_9_125_NWP1_1day$Results[, mean(coef_NWP1_90), by = lead_time], type = 'l', ylab = expression("NWP " * beta *  " -coefficient"))
lines(Interval_Mod_9_125_NWP1_2day$Results[, mean(coef_NWP1_90), by = lead_time], col = 1)
...
lines(Interval_Mod_9_125_NWP1_32day$Results[, mean(coef_NWP1_90), by = lead_time], col = 5)
