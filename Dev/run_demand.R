...
mod_list = seq(50, 150, 10)

for (nr_rounds in mod_list){
    xgb_func = function(dt_train, dt_demand, I_test, X_mat_train, X_mat_test, m){
...
for (nr_rounds in mod_list){
    xgb_func = function(dt_train, dt_demand, I_test, X_mat_train, X_mat_test, m){
        ...
        dt_test$season = factor(dt_test$season)
        dt_test = dt_test[I_test,]
        dt_mod_test = model.matrix(~ hour:week + w_day:month + month + season + year + volume, data = dt_test)
        xgb_test = data.table(dt_mod_test, X_mat_test)
